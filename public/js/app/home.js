(function($,ns) {
	...
	ns.MainUserView = Backbone.View.extend({
		...
		initialize: function(options) {
			...
			var history_ids = this.history.pluck('history_id');
			q.mustBeOneOf('history', history_ids);

			var users = new StackMob.Users();
			users.query(q, {
				success: _.bind(function(result) {

					var totalCnt = history_ids.length+1,
						countArr = [0,0,0,0,0,0,0,0,0];

					_.each(result.models, function(itm) {
						var bucket = Math.floor((_.intersection(itm.get('history'), history_ids).length/totalCnt)*10)-1;
						if (bucket < 0) { bucket = 0; }
						countArr[bucket]++;
					}, this);

					this.updateCount(countArr.reverse());

				},this)
			});
		},

		updateCount: function(countArr) {
			this.$(".tabs-left .nav-tabs .badge").each(function(i) {
				if (countArr[i]) {
					$(this).text(countArr[i]).removeClass("hidden");
				}
			});
		}
	...
	ns.UserProfileEditView = Backbone.View.extend({
			...
			try {
				...
				existing_movies.query(mq, {
					success: _.bind(function(existing) {
						var existing_ids = existing.pluck("movie_id"),
							new_movies = _.difference(profile_ids,existing_ids).map(function(mid){return _self.profile.get(mid)});
						...
						if (existing_ids.length>0) {
							this.user.appendAndSave("profile", existing_ids);
							existing.each(function(exi){ if(exi.hasChanged()){exi.save();}});
						}
						this.$el.modal('hide');	
					},this)
				});
		...
		bindProfileEvents: function(profile) {
			...
			this.profile.bind("add", this.profileInitAddOne, this);
			this.profile.bind("reset", this.profileInitAddAll, this);

			var q = new StackMob.Model.Query();
			q.setExpand(1);
			q.select('profile').select('profile.*');
			this.user.query(q, {
				success: _.bind(function(usr) {
					var pros = _.map(usr.get('profile') || [],function(p){return new ns.MovieModel(p);});
					if (pros.length>0) {
						this.profile.add(pros);
					}
					this.profile.unbind("add", this.profileInitAddOne, this);
					this.profile.unbind("reset", this.profileInitAddAll, this);
		...
		profileInitAddOne: function(movie) {
			var nd = this.$("#movie_profile .thumbnail."+movie.get('rating')).get(movie.get('ratingidx'));
			movie.bind("change:ratingidx", this.shiftBoxArt, this);
			movie.bind("change:rating", this.onRatingChange, this);
			this.setBoxArtImg($(nd), movie);
		},

		profileInitAddAll: function(movies) {
			this.profile.each(_.bind(this.profileInitAddOne,this));
		},
		...
		profileAddAll: function(movie) {
			this.profile.each(_.bind(this.profileAddOne,this));
		},
		...
		onAddMovie: function(e) {
			try {
				...
				mv.movie_id = mv.history_id +":"+ this.hashCode(this.user.get('username'));
				...
				success: _.bind(function(data) {
					if (data && data.response) {
						...
						if(newHistory.length > 0) {
							...
							this.addOrUpdate(new ns.MovieHistoryCollection(newHistory));
		...
		addOrUpdate: function(newHistory) {
			var history_ids = newHistory.pluck('history_id'),
				existing_history = new ns.MovieHistoryCollection(),
				q = new StackMob.Collection.Query();

			q.mustBeOneOf('history_id', newHistory.pluck("history_id"));
			existing_history.query(q, {
				success: _.bind(function(existing) {
					var existing_ids = existing.pluck("history_id"),
						newHistoryCreate = _.difference(history_ids,existing.pluck("history_id")).map(function(hd){return newHistory.get(hd);});
	
					if (newHistoryCreate.length>0) {
						this.user.addRelationship('history', newHistoryCreate);
					}
					if (existing_ids.length>0) {
						this.user.appendAndSave('history', existing_ids);
					}
				},this)
			});
